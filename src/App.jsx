...
// ⚠️ REPLACE THESE with your Supabase project credentials (Settings → API)
...
// Writes to both localStorage (instant) and Supabase (background sync)
...
const loadStorage = async (k) => {
  // Try Supabase first (source of truth), fall back to localStorage
  try {
    const { data } = await supabase.from("planner_data").select("value").eq("key", k).single();
    if (data?.value !== undefined && data?.value !== null) {
      localStorage.setItem(k, JSON.stringify(data.value)); // sync local cache
      return data.value;
    }
  } catch {}
